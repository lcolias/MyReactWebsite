...
function Blob(props){
    ...
    useEffect(() => {
        const timeout = setTimeout(() => {
            ...
            if(activeIndex > POSITION_STATES - 1) setActiveIndex(0);
    ...
    //  State of blob position
    const [activePos, setActivePosition] = useState(0);

    // Blob Movement Animation

    const positionProps = useSpring({
        pos: blobViewPositions[activeIndex],
        config: {duration:DURATION} 
     });
    ...
    useEffect(() => {
        const timeout = setTimeout(() => {
            setActiveIndex(activePos + 1);            
        }, DURATION);

            if(activePos > BLOB_STATES - 1) setActivePosition(0);
